  ...
  return (
    ...
    >
      ...
      <div className="border-t border-neutral-200 p-4">
        <div className="mb-1 flex items-center gap-2 text-[#33322e]">
          <FolderClosed className="size-4 text-neutral-600" aria-hidden />
          <h3 className="truncate text-[15px] font-semibold">{title}</h3>
        </div>
        {subtitle && (
          <p className="mb-3 line-clamp-2 text-sm text-neutral-600">{subtitle}</p>
        )}
        {tag && <Chip>{tag}</Chip>}
      </div>
    </a>
...
export default function Projects() {
  ...
  return (
    <section aria-labelledby="projects-heading" className="mt-[clamp(28px,3.5vw,56px)]">
      <h2
        id="projects-heading"
      ...
      <div className="mt-3 border-b border-neutral-200">
        <div role="tablist" aria-label="Project categories" className="flex items-center gap-6">
          {tabs.map((t, i) => (
            <TabButton
              key={t.key}
              id={t.key}
